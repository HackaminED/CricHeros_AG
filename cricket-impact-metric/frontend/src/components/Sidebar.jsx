...
export default function Sidebar({ open = true, onCollapse }) {
  return (
    ...
    >
      {/* Logo */}
      <div className="px-5 py-6 border-b border-[var(--muted)]/40">
      ...
      <nav className="flex-1 px-3 py-4 space-y-1" aria-label="Primary">
        {navItems.map(({ to, icon, label }) => (
          <NavLink
            ...
            to={to}
            className={({ isActive }) =>
              `flex items-center gap-2 px-4 py-3 rounded-[var(--radius-lg)] text-sm font-medium transition-all duration-200 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-3 focus-visible:outline-[rgba(0,78,152,0.5)] ${
                isActive
      ...
      <div className="px-3 py-4 border-t border-[var(--muted)]/40 space-y-4">
        {open && (
