...
const AnimatedList = ({
  ...
  const handleItemMouseEnter = useCallback((index) => {
    if (onItemSelect) setSelectedIndex(index);
  }, [onItemSelect]);
  ...
  return (
    <div className={`scroll-list-container ${className}`}>
      <div 
        ref={listRef} 
        className={`scroll-list ${listContainerClassName} ${!displayScrollbar ? 'no-scrollbar' : ''}`} 
        onScroll={handleScroll}
      >
        {items.map((item, index) => (
          <AnimatedItem
            key={item.id || item.player || item.match_id || index}
            delay={Math.min(index * 0.05, 0.5)} // Cap staggered delay at 0.5s
            index={index}
